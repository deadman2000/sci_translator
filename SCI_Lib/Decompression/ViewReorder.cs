...
namespace SCI_Translator.Decompression
{
    ...
    {
        public static byte[] ReorderView(byte[] data)
        {
            ...
            byte[] celCounts = new byte[lhPresent];
            seeker.Read(celCounts, 0, lhPresent);
            ...
            if (palOffset > 0)
            {
                ...
                var buff = new byte[4 * 256 + 4];
                seeker.Read(buff, 0, buff.Length);
