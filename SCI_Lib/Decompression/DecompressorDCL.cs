...
namespace SCI_Translator.Decompression
{
    class DecompressorDCL : Decompressor
    {
        ...
        const int HUFFMAN_LEAF = 0x40000000;

        ushort huffman_lookup(int[] tree)
        {
            int pos = 0;
            ...
            return (ushort)(tree[pos] & 0xFFFF);
        }

        // Branch node
        static int BN(int pos, int left, int right) => (left << 12) | right;

        // Leaf node
        static int LN(int pos, int value) => (value) | HUFFMAN_LEAF;


        static readonly int[] length_tree = new int[] {
            BN(0, 1, 2),
            BN(1, 3, 4),     BN(2, 5, 6),
        ...
        static readonly int[] distance_tree = new int[] {
            BN(0, 1, 2),
            BN(1, 3, 4),       BN(2, 5, 6),
			//
        ...
        static readonly int[] ascii_tree = new int[] {
